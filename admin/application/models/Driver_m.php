...
//    ==========================================================================================
    ...
    function getJumlahDriverOn() {
        $query = $this->db->query("
SELECT COUNT(id_driver) AS jumlah FROM `config_driver` WHERE `status` = '1'
            ");
        return $query->result_array();
    }
    
    
        function getJumlahDriverWork() {
        $query = $this->db->query("
SELECT COUNT(id_driver) AS jumlah FROM `config_driver` WHERE `status` = '3'
            ");
        return $query->result_array();
    }
...
//        query update status 3 to 5 
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "/api/book/send_cancel_signal/" . $idDriver . "/" . $idPelanggan . "/" . $idTrans);
...
//        lihat tranasksi tanpa limit 15
        ...
        function getTotalTransaksiHarian($hari, $bulan) {
//        lihat tranasksi tanpa limit 15
        $query = $this->db->query("
                SELECT COUNT(nomor) as jumlah
                FROM history_transaksi
                WHERE DAY(waktu) = $hari
